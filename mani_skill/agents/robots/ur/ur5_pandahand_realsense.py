...
@register_agent()
class UR5PandahandRealsense(UR5):
    uid = "ur5_pandahand_realsense"
    urdf_path = f"{PACKAGE_ASSET_DIR}/robots/ur/ur5_pandahand_realsense.urdf"
    urdf_config = dict(
        _materials=dict(
    ...
    keyframes = dict(
        rest=Keyframe(
            pose=sapien.Pose(),
            # qpos=np.array([0, -1.5708, 1.5708*1.1, -1.5708*1.1, -1.5708, 3.1416, 0.04, 0.04]),
            qpos=np.array([-2.9392,  4.3871, -1.6020,  5.0165, -4.6887,  3.3661, 0.04,  0.04]),
            # qpos=np.array([-0.1584, -6.2108, -0.8892, -3.8387,  1.5760,  6.1476, 0.04,  0.04]),
        )
    )
