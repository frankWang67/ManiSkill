...
@register_agent()
class SawyerRobotiqWristCamera(BaseAgent):
    uid = "sawyer_robotiq_wristcam"
    urdf_path = f"{PACKAGE_ASSET_DIR}/robots/sawyer/sawyer_robotiq_wristcam.urdf"
    urdf_config = dict(
        _materials=dict(
    ...
    def _after_loading_articulation(self):
        ...
        # Filter the specific self-collision pairs that destabilize Sawyer at
        # reset, while still leaving the rest of self-collision checking on.
        upper_body_filter_links = ["head", "right_l0", "right_l1", "right_l2", "right_l1_2"]
        for link_name in upper_body_filter_links:
            self.robot.links_map[link_name].set_collision_group_bit(
                group=2, bit_idx=30, bit=1
            )

        forearm_filter_links = ["right_l4_2", "right_l5"]
        for link_name in forearm_filter_links:
            self.robot.links_map[link_name].set_collision_group_bit(
                group=2, bit_idx=29, bit=1
            )
