...
from diffusion_policy.utils import replace_submodules
...
class TimmObsEncoder(ModuleAttrMixin):
    ...
    def forward(self, image):
        BH = image.shape[0]
        assert image.shape[1:] == self.key_shape_map[self.rgb_keys[0]]
        img = self.key_transform_map[self.rgb_keys[0]](image)
        raw_feature = self.key_model_map[self.rgb_keys[0]](img)
        feature = self.aggregate_feature(raw_feature)
        assert len(feature.shape) == 2 and feature.shape[0] == BH
        return feature.reshape(BH, -1)
    ...
    def output_shape(self):
        ...
        example_obs = torch.zeros(
            (1,) + self.key_shape_map[self.rgb_keys[0]], 
            dtype=self.dtype,
            device=self.device
        )
        example_output = self.forward(example_obs)
